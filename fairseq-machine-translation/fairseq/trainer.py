...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion):
        ...
        self.adv_wdecay = args.adv_wdecay
        self.adv_updates = args.adv_updates
    ...
    def discriminator_accuracy(self):
        # rare 1 pop 0
        adv_h = self.model.adv_hidden(self.model.decoder.embed_tokens.weight)
        predicted_label = torch.argmax(adv_h, 1)
        correct = (predicted_label == self.model.adv_targets).sum()
        all = self.model.decoder.embed_tokens.num_embeddings
        print("discriminator accuracy: {1}", (correct.data.tolist() + 0.01) / (all + 0.01))
    ...
    def train_step(self, sample, update_params=True):
        ...
        adv_params = (np[1] for np in self.model.named_parameters() if np[0] == 'adv_hidden.weight'
                      or np[0] != 'adv_hidden.bias')
        self.adv_optimizer = torch.optim.SGD(adv_params, lr=self.adv_lr, weight_decay=self.adv_wdecay)
        ...
        if update_params:
            ...
            try:
                ...
                for idx in range(self.adv_updates):
                    adv_h = self.model.adv_hidden(self.model.decoder.embed_tokens.weight)
                    adv_loss1 = adv_criterion(adv_h[0:self.model.adv_bias],
                                                  self.model.adv_targets[0:self.model.adv_bias])
                    adv_loss2 = adv_criterion(adv_h[self.model.adv_bias:],
                                                  self.model.adv_targets[self.model.adv_bias:])
