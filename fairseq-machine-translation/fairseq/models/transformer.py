...
class TransformerModel(FairseqModel):
    def __init__(self, args, encoder, decoder):
        ...
        #self.adv_bias_enc = args.adv_bias_enc
        #self.adv_hidden_enc = nn.Linear(encoder.embed_tokens.embedding_dim, 2).cuda()
        #self.adv_targets_enc = torch.LongTensor(np.array([0] * self.adv_bias_enc + [1] * (encoder.embed_tokens.num_embeddings - self.adv_bias_enc))).cuda()
        #self.adv_targets_enc = Variable(self.adv_targets_enc)
        #self.adv_hidden_enc.weight.data.uniform_(-0.1, 0.1)

        self.adv_bias = args.adv_bias
        self.adv_hidden = nn.Linear(decoder.embed_tokens.embedding_dim, 2).cuda()
        self.adv_targets = torch.LongTensor(np.array([0] * self.adv_bias + [1] * (decoder.embed_tokens.num_embeddings - self.adv_bias))).cuda()
        self.adv_targets = Variable(self.adv_targets)
        self.adv_hidden.weight.data.uniform_(-0.1, 0.1)
